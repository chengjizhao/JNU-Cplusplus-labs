...
/*总结；基本功能都可以实现，但是由于技术力不够，将代码都实现在main函数中。
这就导致了程序的实现要按照我的代码顺序来，而不是按用户的要求来。实现这样的功能应该把各部位切割为函数，
并在main函数中只写选择，按照用户的选择来运行实现具体功能的函数。函数的变量使用范围我暂时不够清晰，
导致我无法任意使用studentvector这一矢量数组，使函数的切割无法使用，再提升。文件的读取和写入都非常有意思，
但是学习成本确实过高，实现功能后很难有时间再深读*/
