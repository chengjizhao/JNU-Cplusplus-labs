...
void random_withdraw(std::vector<int>* accounts, int index) {
  ...
  std::cout << "Account " << index << " now has $" << (*accounts)[index]
            << std::endl;
  mtx.unlock();
}
...
void do_something_in_random_period(void (*fn)(std::vector<int>*, int), int n,
                                   std::vector<int>* v) {
  for (int i = 0; i < n; ++i) {
    fn(v, rand() % 9);
...
int main() {
  ...
  std::thread withdraw_thread(do_something_in_random_period, random_withdraw,
                              10, &accounts);
  std::thread deposit_thread(do_something_in_random_period, random_deposit, 10,
                             &accounts);
