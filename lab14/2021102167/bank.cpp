...
int main() {
    ...
    std::thread depositThread([&account, &generator]() { depositThread(account, generator); });
    std::thread withdrawThread([&account, &generator]() { withdrawThread(account, generator); });
    depositThread.join();
    withdrawThread.join(); 
    return 0;
}
